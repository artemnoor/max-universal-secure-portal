...
test('test config is safe and frozen', () => { const config = loadConfig({ NODE_ENV: 'test' }); assert.equal(config.transport, 'polling'); assert.equal(config.initDataTtlSeconds, 900); assert.ok(Object.isFrozen(config)); });
